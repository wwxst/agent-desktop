...
Local Speech Understanding      本地语音理解             FFmpeg 提取标准 WAV，whisper.cpp 本地转录，DeepSeek 根据 transcript 继续理解；Tool 接入与自动化验证已完成，真实 whisper.cpp 与端到端验证待完成。
```
...
本地语音 Tool 接入、自动化测试和真实 FFmpeg WAV 提取已验证。真实 whisper.cpp 转录与 DeepSeek + Local Speech 端到端验证需要仓库外准备官方 whisper.cpp `whisper-cli` 和 `ggml-small.bin`，当前尚未完成。
