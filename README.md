...
普通问答和文字任务不以视频附件为前提。模型可以用 `set_working_directory` 请求会话工作目录，经用户确认后，用 `list_directory` 发现文件、用 `read_file` 按行读取文本、用 `search_text` 按字面文本搜索内容；所有读取只限已确认的目录，越界会明确要求重新指定工作目录。
