...
Speech Understanding            语音理解                 Tool 接入与自动化验证已完成；真实 Speech API 与端到端验证待完成。
```

# Current Engineering Foundation（当前工程基础）

项目采用 Node.js 24 LTS、pnpm workspace、TypeScript ESM 和 Vitest。当前 Agent Runtime 已具备核心接口、最小 Agent Loop、可运行 Echo Agent、DeepSeek 真实模型适配器、FFmpeg 视频处理、视觉理解、语音 Tool 接入和基于内容的剪辑能力。
...
语音理解的 Tool 接入、自动化测试和真实 FFmpeg 音频提取已验证。由于当前 endpoint 不可用，真实 Speech API 以及 DeepSeek + Speech 端到端验证尚未完成。
