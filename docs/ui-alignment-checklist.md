...
- [x] 完成本轮已有组件调整并连接原业务；未实现的搜索、预览不提供占位控件。
...
| 02 侧栏骨架 | `ui-sidebar/src/client/SidebarRoot.module.css`、`ui-layout` | `App.tsx`、`styles.css`；侧栏由 240 调为参考桌面宽度；导航字号与新会话顺序已调整；移除无操作价值的“本地运行”状态行 | 已对齐 | 已实现 | 保留纯文字品牌与独立会话标签；“设置”是侧栏最底部的唯一操作；工作区名称行已整块移除；窄侧栏宽度差异见下文 | 桌面/窄窗口与 40 会话均可达，设置底边位置有浏览器断言；E1/E7 |
...
| 04 会话搜索 | `ui-workspace/src/client/rows/WorkspaceBrowser.tsx` | 当前不呈现搜索控件 | 待处理 | 待实现 | 后续本地标题/消息过滤 | 未实现能力不提供占位入口；E1 |
...
| 08 智能体正文与流式输出 | `ui-chat/src/client/chat/AssistantMarkdown.tsx` | `AssistantMessage.tsx`、`Markdown.tsx`；正文和流式增量共用最小 Markdown 渲染 | 已对齐 | 已实现 | 支持段落、列表、代码、加粗和 web 链接；不支持的语法保持原文 | 单元测试覆盖最终回复、流式增量、长代码和非 web 链接；E4 |
...
| 14 活动摘要与展开 | `ui-chat/src/client/chat/TurnProcessNodeView.module.css` | `AgentActivity.tsx`；摘要细分隔、字号、矢量箭头和紧凑行已调整 | 已对齐 | 已实现 | 保持运行展开、终态折叠；参考嵌套过程另列 16 | 参考展开/折叠与当前 8 项活动对照；E3/E7 |
| 15 活动运行/成功/失败/停止 | `ui-tool/src/client/tool/components/ToolRow.module.css` | `AgentActivity.tsx`；使用矢量状态图形，并展示真实状态、耗时和文件引用 | 进行中 | 已实现 | 当前各状态已验；参考实时处理中/取消态尚未取得同状态截图 | 成功/失败参考可见；当前四种状态可见；不把缺少参考证据算通过；E3/E4 |
...
| 18 视频内嵌预览 | `ui-deliverables/src/client/PresentedFileCard.tsx` 的预览入口 | 当前产物卡只提供真实“打开文件”操作 | 待处理 | 待实现 | 播放器依赖桌面媒体访问协议与格式支持 | 未实现能力不提供占位入口；E3/E4 |
...
| 20 设置加载、保存、失败、禁用 | `ui-settings-models`、`ui-settings-general` | `RuntimeSettingsPanel.tsx`；修复加载错误被加载文案遮挡；已配置密钥在输入框内显示固定八位黑色星号掩码，真实值不进入渲染层；保存/清除错误使用警告语义 | 已对齐 | 已实现 | 沿用真实保存接口；掩码不作为新密钥提交；不复制参考配置文件/权限设置 | 加载、加载失败、固定掩码、新密钥替换、未编辑保存及处理中禁用通过；保存失败截图未单独覆盖；E5/E6 |
...
- 当前保留纯文字品牌和会话标签；诊断编号只写入本地 Trace，不在界面显示。成功回复不再显示“角色标记 + Agent”标题行，只保留“已停止”“任务失败”终态标记。不复制参考顶部栏、独立轨迹页、权限/模型快捷选择、用量页脚。参考新会话必须选工作区，当前支持直接发送纯文本。侧栏原有的工作区名称行（分组标题加文件夹行）没有真实消费者，已整块移除。
- 窄窗口当前保留会话按钮及操作列，侧栏宽于参考；设置导航收窄以保证字段可编辑。导航采用现有四个设置分区，未迁移参考分类或插件。
- 侧栏底色有意深于参考：参考的侧栏填充为 `rgb(249, 250, 251)`，当前使用 `#f2f2f2`，让菜单面板与白色主工作区有更明确的分离。这是产品选择，不是对齐遗漏，不要按参考改回。
- 搜索与预览不提供占位入口；拖入、工具详情、外观偏好、复制/时间仍有缺口，不能视为整项完成。
...
2. 附件拖入：依赖桌面路径解析与当前附件契约；不得用浏览器文件对象伪装本地路径。
