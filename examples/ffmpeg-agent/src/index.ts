...
async function runCli(
  ...
  const agent: Agent = {
    ...
    systemPrompt: new StaticSystemPrompt([
      ...
      '当用户要求了解视频中的对白、旁白、口播或主要语音内容时，使用 extract_audio 生成标准 WAV，再使用 transcribe_audio 获取文字 transcript，然后根据 transcript 回答或继续推理。',
      'transcribe_audio 只负责把 WAV 转成文字，不负责总结、剪辑决策、时间戳或字幕生成。',
